<template>
  <div class="detection-container">
    <!-- 标题 -->
    <h1 class="page-title">导师画像建模列表</h1>
...
<script>
import { fetchSupervisorProfile } from "@/api/supervisorProfile";
import { mapState } from "vuex";

export default {
  name: "SupervisorProfileAnalysisView",
  computed: {
    ...mapState({
  ...
  methods: {
    async fetchModelList() {
      ...
      this.loading = true;
      try {
        const response = await fetchSupervisorProfile({
          userId: this.userId,
        });
    ...
    navigateToDetail(id) {
      this.$router.push({
        path:
          "/supervisor-profile/interpretation/supv/" +
          this.tableData[id].modelId,
        params: {
