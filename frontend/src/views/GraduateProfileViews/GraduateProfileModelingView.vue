<template>
  <div class="modeling-container">
    <!-- 标题 -->
    <h1 class="page-title">研究生画像宽表</h1>
...
import { generateGraduateProfile } from "@/api/graduateProfile";
export default {
  name: "GraduateProfileModelingView",
  computed: {
    ...mapState({
  ...
  data() {
    return {
      ...
      tableData: [],
      selectedItems: [],
      modelId: "graduate_profile_20250410_132147",
    };
  },
  async mounted() {
    try {
  ...
  methods: {
    ...
    async startModeling() {
      ...
      try {
        // 异步发送建模请求
        const modelingPromise = generateGraduateProfile(params);
    ...
    viewModel() {
      this.$router.push({
        path: "/graduate-profile/interpretation/pg/" + this.modelId,
        params: {
