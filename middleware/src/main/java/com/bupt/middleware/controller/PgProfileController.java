...
import com.bupt.middleware.entity.*;
import com.bupt.middleware.entity.result.Result;
import com.bupt.middleware.service.IPgProfileService;
 ...
 * @author chenxiao
 * @date 2025/4/9 11:39
 * @description: Postgraduate Profile Controller
 */

@RestController
@RequestMapping("/profile/pg")
@Slf4j
public class PgProfileController {
     ...
     */
    @PostMapping("/build")
    public Result<String> buildModel(@RequestBody PgModelBuildDTO pgModelBuildDTO) throws IllegalAccessException {
        log.info("Starting to build profile model for user: {}", pgModelBuildDTO.getUserId());

        String userId = pgModelBuildDTO.getUserId();
        PgWideTableAttributes pgWideTableAttributes = pgModelBuildDTO.getPgWideTableAttributes();
        ...
        log.info("Fetched {} postgraduate records for clustering", pgWideTables.size());
        ...
        resultList = pgProfileService.buildPgProfile(attList, pgWideTables, null);
        log.info("Clustering completed. Result contains {} records", resultList.size());

        // 确定命名 graduate_功能类型_日期_时间
        String profileId = ModelName.getModelName("postgraduate", "profile");
        ...
        PgProfileRecord pgProfileRecord = new PgProfileRecord();

        pgProfileRecord.setFilePath(relativePath + profileId + ".json");
        pgProfileRecord.setUserId(userId);
        pgProfileRecord.setProfileId(profileId);
        pgProfileRecord.setAlgorithmType("k-means");
        pgProfileRecord.setCreateTime(localDateTime);
        pgProfileRecord.setAttributeAmount(String.valueOf(attList.size()));
        pgProfileRecord.setAttributeList(attList.toString());

        log.info("Saving profile record to database: {}", pgProfileRecord);
        Result<String> saveResult = pgProfileService.postPgProfileRecord(pgProfileRecord);
     ...
     * 获取画像结果列表
     * param: userId
     * return: Result<List<PgProfileRecord>>
     */
    @GetMapping("/getList")
    public Result<List<PgProfileRecord>> getRecords(@RequestParam(value = "userId") String userId) {
        log.info("Fetching profile records for user: {}", userId);
        Result<List<PgProfileRecord>> result = pgProfileService.getPgProfileRecords(userId);
     ...
     * 根据profileId获取单条画像结果
     * param: profileId
     * return: Result<PgProfileRecord>
     */
    @GetMapping("/getRecord")
    public Result<PgProfileRecord> getRecord(@RequestParam(value = "profileId") String profileId) {
        log.info("Fetching profile record with ID: {}", profileId);
        Result<PgProfileRecord> result = pgProfileService.getPgProfileRecord(profileId);
    ...
    public Result<List<Map<String, Object>>> getClusterDistribution(@RequestParam(value = "profileId") String profileId) {
        ...
        try {
            ...
            Result<PgProfileRecord> recordResult = pgProfileService.getPgProfileRecord(profileId);
            ...
            PgProfileRecord record = recordResult.getData();
    ...
    public Result<List<Map<String, Object>>> getAttrBinning(@RequestParam(value = "profileId") String profileId,
        ...
        try {
            ...
            log.info(clusterData.get(0).toString());

            // 4. 检查属性是否存在
            if (!clusterData.get(0).containsKey(attributeId)) {
