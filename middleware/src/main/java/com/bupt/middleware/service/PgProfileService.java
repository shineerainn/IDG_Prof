 ...
 * @author chenxiao
 * @date 2025/4/9 16:15
 * @description: Postgraduate Profile Cluster Service
 */
...
public class PgProfileService implements IPgProfileService {
     ...
     */
    @Override
    public List<Map<String, Object>> buildPgProfile(List<String> attrList, List<PgWideTable> pgWideTables, AlgorithmConfig algorithmConfig) {
        // 数据转json
        ...
        url = "http://127.0.0.1:8009/cluster/buildStuCluster";
     ...
     */
    @Override
    public Result<List<PgProfileRecord>> getPgProfileRecords(String userId) {
        List<PgProfileRecord> pgProfileRecords;
        ...
        try (HttpClient client = HttpClient.newHttpClient()) {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(databaseUrl + "/database/pgProfile/userId/" + userId))
                    .GET()
                    .build();
            ...
            // 解析响应
            ResultDTO<List<PgProfileRecord>> resultDto = objectMapper.readValue(response.body(), new TypeReference<>() {});

            if (resultDto.getCode() == 200) {
                pgProfileRecords = resultDto.getData();
            } else {
        ...
        return Result.success(pgProfileRecords, "success");
    }
     ...
     */
    @Override
    public Result<PgProfileRecord> getPgProfileRecord(String profileId) {
        PgProfileRecord pgProfileRecord;
        ...
        try (HttpClient client = HttpClient.newHttpClient()) {
            ...
            // 创建HTTP GET请求
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(databaseUrl + "/database/pgProfile/profileId/" + profileId))
                    .GET()
                    .build();
            ...
            // 解析响应
            ResultDTO<PgProfileRecord> resultDto = objectMapper.readValue(response.body(), new TypeReference<>() {});

            if (resultDto.getCode() == 200) {
                pgProfileRecord = resultDto.getData();
            } else {
                return Result.error(resultDto.getCode(), resultDto.getMessage());
            }
        ...
        return Result.success(pgProfileRecord, "success");
    }
     ...
     */
    @Override
    public Result<String> postPgProfileRecord(PgProfileRecord pgProfileRecord) {
        try(HttpClient client = HttpClient.newHttpClient()) {
            // Jackson JSON 解析器
            ObjectMapper objectMapper = new ObjectMapper();
            // 查找并自动注册Jackson模块，用于支持DateTime的序列化
            ...
            // 构造请求体
            String requestBody = objectMapper.writeValueAsString(pgProfileRecord);

            // 创建HTTP PUT请求
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(databaseUrl + "/database/pgProfile"))
